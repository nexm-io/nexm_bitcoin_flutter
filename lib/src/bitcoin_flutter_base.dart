...
class HDWallet {
  ...
  String get privKey {
    return HEX.encode(_bip32.privateKey as Uint8List);
  }
  ...
  HDWallet(
      {required bip32.BIP32 bip32,
      required P2PKH p2pkh,
  ...
  factory HDWallet.fromSeed(Uint8List seed, {NetworkType? network}) {
    ...
    final wallet = bip32.BIP32.fromSeed(
        ...
        bip32.NetworkType(
            ...
            wif: network.wif));
    final p2pkh = P2PKH(
        data: PaymentData(pubkey: wallet.publicKey),
        network: network);
    return HDWallet(
...
class Wallet {
  late ECPair _keyPair;
  late P2PKH? _p2pkh;
  NetworkType? network;

  String? get privKey => HEX.encode(_keyPair.privateKey as Uint8List);
  ...
  Wallet._(this._keyPair, this._p2pkh, this.network);
  ...
  factory Wallet.random([NetworkType? network]) {
    ...
    final _p2pkh = P2PKH(
        ...
        network: network);
    return Wallet._(_keyPair, _p2pkh, network);
  }
